...
<p style = "text-align: :right;color:gray;"><i>{{$item->created_at}}</i></p>
    ...
    <a class = "btn btn-success" href="{{ route('edit', ['id'=>$item->id])}}">Editer</a>
    <a class = "btn btn-danger" onclick="supprimer(event)"data-bs-toggle="modal" data-bs-target="#exampleModal" href="{{ route('destroy', ['id'=>$item->id])}}">Supprimer</a></p>
